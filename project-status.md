...
### 用户名唯一性检查功能
1. 修改文件：
   - sql_model/tb_user.py
     - 为用户表的username字段添加唯一性约束(unique=True)
   - backend/user_manage_backend.py
     - 在添加用户时增加用户名重复检查
     - 在编辑用户时增加用户名重复检查
     - 优化了用户名重复时的错误提示信息
...
2. 主要变更：
   - 数据库层面确保用户名唯一性
   - 添加用户时会检查用户名是否已存在
   - 编辑用户时会检查新用户名是否与其他用户重复
   - 添加了相应的日志记录
   - 优化了错误提示信息的用户体验

### 应激评估界面进度条优化
1. 修改文件：
   - backend/health_evaluate_backend.py
     - 移除单个数据评估时的进度条显示
     - 移除单个数据评估时的进度条更新
     - 保留批量评估时的进度条功能
...
1. 修改文件：
   - backend/health_evaluate_backend.py
     - 修改 checkButton 方法，增加评估结果显示功能
     - 在查看数据时显示三种应激类型的评估分数
     - 根据评估结果更新LED灯的状态（红色/灰色）
     - 当无评估结果时清空分数显示
...
### 2024-03-22 导出功能修复
1. 修复了结果管理中导出功能的错误
   - 安装了openpyxl模块（版本3.1.5）
   - 解决了"No module named 'openpyxl'"错误
   - 现在可以正常导出Excel文件
...
## 2024-03-21
- 在结果管理界面(front/results_manage_UI.py)添加了滚动条功能
  - 添加了QScrollArea作为主容器
  - 实现了内容超出窗口时的滚动功能
  - 保持了原有布局和样式不变
  - 滚动条会根据需要自动显示/隐藏
