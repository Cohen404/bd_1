...
### 更新内容
1. 适配新的数据库结构
   - 添加了 Role 和 RolePermission 模型支持
   - 更新了数据库字段的使用

2. 增强权限管理系统
   - 添加了用户权限检查功能
   - 基于用户类型（admin/user）的权限控制
   - 使用 user_id 替代 username 进行用户识别
...
## 2024-03-22 角色管理权限更新
### 修改内容
1. 修改了数据库初始化脚本 `util/init_db.py`：
   - 将"数据采集"权限改为"应激评估"权限
   - 修改权限ID从 `PERM_DATA_COLLECTION` 改为 `PERM_STRESS_ASSESSMENT`
   - 更新权限名称、页面URL和描述
   - 更新了管理员和普通用户的权限分配

### 主要变更
- 权限ID: PERM_STRESS_ASSESSMENT
- 权限名称: 应激评估
- 页面URL: /stress_assessment
- 权限描述: 应激评估页面访问权限
- 权限分配:
  * 管理员角色保持全部权限
  * 普通用户角色保持基本权限
...
## 2024-03-22 应激评估权限显示修复
### 修改内容
1. 修复了管理员界面和普通用户界面中应激评估按钮不显示的问题：
   - 修改了 `backend/admin_index_backend.py`：
     * 将权限ID从 `PERM_DATA_COLLECTION` 更新为 `PERM_STRESS_ASSESSMENT`
   - 修改了 `backend/index_backend.py`：
     * 将权限ID从 `PERM_DATA_COLLECTION` 更新为 `PERM_STRESS_ASSESSMENT`

### 主要变更
- 更新了权限检查逻辑，使用新的权限ID
- 确保管理员和普通用户界面都能正确显示应激评估按钮
- 保持了原有的按钮布局和显示逻辑不变
...
## 2024-02-16 上传服务用户名统一

修改了上传服务(`backend/upload_service.py`)的用户标识方式：
...
2. 数据库记录字段更新：
   - personnel_id 使用用户名
   - user_id 使用用户名
   - personnel_name 使用用户名
   - 保持其他字段不变

主要修改：
- 修改了 `check_user_permission` 函数的参数和逻辑
- 更新了数据记录创建时的字段赋值
- 保持了文件上传和处理逻辑不变
...
## 2024-02-16 安装脚本数据库配置更新

更新了安装脚本(`install.sh`)的数据库配置：
