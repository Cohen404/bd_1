...
def attention_block(in_layer, attention_model, ratio=8, residual=False, apply_to_input=True):
    ...
    # 根据输入特征的维度和输出特征的维度调整输出特征的形状，确保形状匹配
...
def mha_block(input_feature, key_dim=8, num_heads=2, dropout=0.5, vanilla=True):
    ...
    """
    # Layer normalization
    x = LayerNormalization(epsilon=1e-6)(input_feature)  # 层归一化
    ...
    if vanilla:
        ...
        # Create a multi-head local self-attention layer as described in  多头局部自注意力
        # 'Vision Transformer for Small-Size Datasets' https://arxiv.org/abs/2112.13492v1

        # Build the diagonal attention mask 构建对角线注意力掩码
        ...
        x = MultiHeadAttention_LSA(key_dim=key_dim, num_heads=num_heads, dropout=dropout)(
            x, x, attention_mask=diag_attn_mask)
    x = Dropout(0.3)(x)
    # Skip connection 跳跃连接  将输入特征与 dropout 后的特征进行相加，形成跳跃连接
...
class MultiHeadAttention_LSA(tf.keras.layers.MultiHeadAttention):
    ...
    def __init__(self, **kwargs):
        ...
        # root of the key dimension.键维度的平方根
        self.tau = tf.Variable(math.sqrt(float(self._key_dim)), trainable=True)

    def _compute_attention(self, query, key, value, attention_mask=None, training=None):
        query = tf.multiply(query, 1.0 / self.tau)  # 调整注意力分布的尖峰程度
        attention_scores = tf.einsum(self._dot_product_equation, key, query)  # 矩阵乘法计算注意力分数
        attention_scores = self._masked_softmax(attention_scores, attention_mask)  # 将注意力分数与注意力掩码相乘，并进行 softmax 操作
        attention_scores_dropout = self._dropout_layer(
            attention_scores, training=training
        )
        attention_output = tf.einsum(  # 矩阵乘法计算注意力输出
