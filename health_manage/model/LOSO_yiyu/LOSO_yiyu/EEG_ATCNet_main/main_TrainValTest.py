...
# %% Training
def train(dataset_conf, train_conf, results_path,Leave_one):
    '''
    Parameters
    ----------
    ...
    train_conf: {'batch_size': 256, 'epochs': 500, 'patience': 100, 'lr': 0.001, 'n_train': 1,
    ...
    acc = np.zeros((n_sub, n_train))  # 1 * 1
    ...
    for sub in range(n_sub):  # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        X_train, _, y_train_onehot, _, _, _ = get_data(
            data_path, sub,Leave_one, dataset, LOSO=LOSO, isStandard=isStandard)
        ...
        for train in range(n_train):  # How many repetitions of training for subject i.
            ...
            model.compile(loss=CategoricalCrossentropy(from_logits=from_logits), optimizer=Adam(learning_rate=lr),
                          metrics=['accuracy']) ###2 loss
            ...
            """
                verbose=0：静默模式，不打印任何日志信息。
                verbose=1：默认模式，打印进度条，并在每个 epoch 结束时打印一行日志。
                verbose=2：安静模式，在每个 epoch 结束时打印一行日志，但不显示进度条。
                verbose 为其他整数值时，打印更详细的信息，例如每个 batch 的进度。
            """
            ...
            # from_logits = False
            # 如果为 False，表示模型的输出已经是经过 softmax 处理后的概率分布，这时候就可以直接取概率最大的类别作为预测结果。
            if from_logits:
            ...
            print(f"******自己的train输出label和y_pred******\n {labels.shape} \n{labels} \n{y_pred.shape}  \n {y_pred}")
            ...
            info = info + 'valid_acc: {:.4f}   valid_loss: {:.3f}'.format(acc[sub, train],
...
# %% Evaluation
def test(model, dataset_conf, results_path, Leave_one,allRuns=True):
    ...
    for sub in range(n_sub):  # (num_sub): for all subjects, (i-1,i): for the ith subject.
        # Load data
        _, _, _, X_test, _, y_test_onehot = get_data(data_path, sub, Leave_one,dataset, LOSO=LOSO, isStandard=isStandard,isShuffle=False)
    ...
    head2 = head2 + '  -------'
    info = '\n' + head1 + '\n' + head2
    info = '\n---------------------------------\nTest performance (acc & k-score):Leave_one={}\n'.format(Leave_one)
    info = info + '---------------------------------\n' + head1 + '\n' + head2
...
def run(Leave_one):
    ...
    # dataset = 'BCI2a'  # Options: 'BCI2a','HGD', 'CS2R', 'eegdata'
    dataset = 'eegdata'
    ...
    if dataset == 'BCI2a':
        ...
        # 更换路径
        data_path = r'/e/lb/EEG_main/dataset/BCI IV-2a/'
    elif dataset == 'eegdata':
        in_samples = 1000
        n_channels = 59
        n_sub = 1
        n_classes = 2
        classes_labels = ['Control', 'EXP']
        # data_path = os.path.expanduser('~') + '/BCI Competition IV/BCI Competition IV-2a/BCI Competition IV 2a mat/'
        # 更换路径
        data_path = r'..'
    elif dataset == 'HGD':
    ...
    train_conf = {'batch_size': 256, 'epochs': 256, 'patience': 100, 'lr': 0.001, 'n_train': 1,
                  'LearnCurves': True, 'from_logits': False, 'model': 'ATCNet'}

    # pprint(dataset_conf)

    # Train the model
    train(dataset_conf, train_conf, results_path,Leave_one)

    # Evaluate the model based on the weights saved in the '/results' folder
    model = getModel(train_conf.get('model'), dataset_conf)
    test(model, dataset_conf, results_path,Leave_one)


# %%
if __name__ == "__main__":
    for Leave_one in range(1,13):
        run(Leave_one)
